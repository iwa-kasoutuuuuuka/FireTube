...
[📥 **最新の APK をダウンロード (FireTube-v1.3.4.apk)**](https://github.com/iwa-kasoutuuuuuka/FireTube/raw/main/FireTube-v1.3.4.apk) / [リポジトリ内ファイル](FireTube-v1.3.4.apk) / [GitHub Releases](https://github.com/iwa-kasoutuuuuuka/FireTube/releases)
...
| **スマート・フォーカス先読み (Focus-Dwell Prefetch)** | リモコン D-Pad で動画カードに 700ms 滞在した瞬間に、バックグラウンドの非同期コルーチンでストリーム情報を先行取得。高速カーソル移動中は自動キャンセルし、CPU・Wi-Fi帯域を浪費しないゼロ負荷設計。 |
...
| **ExoPlayer LRU ディスクキャッシュ (40MB)** | `SimpleCache` (40MB) によるメディアセグメントのローカルキャッシュ。ライブ配信の動的プレイリスト (`.m3u8`) は完全バイパスし、生放送の更新停止 (`PlaylistStuckException`) を防止しつつシーク・再開を 0ms 化。 |
| **Glide スマート・プリロード (Smart Preload)** | トレンド動画ロード時、画面内に見える上位 6 枚のサムネイル（`hqdefault.jpg`）を先行デコード。初回描画時や画面遷移時のチラつきを根絶。 |
...
## 🏗️ 超低遅延・マルチレイヤー高速化アーキテクチャ

Fire TV Stick HD (1.5GB RAM) のような低スペック SoC 端末において、従来のクライアントは「決定キーを押してからスクレイピングや暗号解読を行い、数秒待たされる」のが最大のストレス要因でした。

FireTube v1.3.4 では、**人間の認知・決定時間（動画カードにカーソルを合わせてから決定キーを押すまでの 0.5〜2 秒）** を活用し、UI・ネットワーク・メディア各層を垂直統合した高速化パイプラインを構築しています：
...
1. 動画カードにフォーカス (D-Pad)
   │
   ├─► 700ms 滞在 (Dwell) ────► 【バックグラウンド先行抽出】
   │                              - FastDns (インメモリ名前解決 0ms)
   │                              - NewPipe / Piped 高速抽出
   │                              - LRU ストリームキャッシュに保存 (有効期限15分)
   ▼
2. 決定 (OK) キー押下！
   │
...
3. ExoPlayer 再生開始
   ...
   │      └─ 動画・音声データ (.mp4, .ts)   ─► 40MB LRU キャッシュ (再開・シーク 0ms)
   ▼
【結果: 実測 0.8秒で爆速再生開始 (TTFF)】
```
